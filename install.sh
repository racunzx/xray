...
check_release(){
    ...
    if ["$RELEASE" == "centos" ]; then
        ...
        yum install -y wget
        if ["$VERSION" == "6"] ;then
            red "$(date +"%Y-%m-%d %H:%M:%S")-CentOS 6.\n== Install failed."
            exit
        fi
        if ["$VERSION" == "5"] ;then
            red "$(date +"%Y-%m-%d %H:%M:%S")-CentOS 5.\n== Install failed."
            exit
        fi
        ...
        if ["$VERSION" == "14"] ;then
            red "$(date +"%Y-%m-%d %H:%M:%S")-Ubuntu 14.\n== Install failed."
            exit
        fi
        if ["$VERSION" == "12"] ;then
            red "$(date +"%Y-%m-%d %H:%M:%S")-Ubuntu 12.\n== Install failed."
            exit
        fi
...
check_port(){
    ...
    if [-n "$Port80" ]; then
        ...
        red "$(date +"%Y-%m-%d %H:%M:%S")-Port 80 is occupied, the process is occupied: ${process80}\n== Install failed."
        exit 1
    fi
    if [-n "$Port443" ]; then
        process443=`netstat -tlpn | awk -F'[: ]+''$5=="443"{print $9}'`
        red "$(date +"%Y-%m-%d %H:%M:%S")-Port 443 is occupied, the process is occupied: ${process443}.\n== Install failed."
        exit 1
    fi
}
install_nginx(){
    green "$(date +"%Y-%m-%d %H:%M:%S") ==== install nginx"
    $systemPackage install -y nginx
    if [! -d "/etc/nginx" ]; then
        red "$(date +"%Y-%m-%d %H:%M:%S")-It seems that nginx has not been installed successfully, please use the delete xray function in the script, and then reinstall.\n == Install failed."
        exit 1
    fi
...
EOF
    ...
    # loggreen "Set Selinux to allow nginx"
    ...
    if test -s /root/.acme.sh/$your_domain/fullchain.cer; then
        ...
        cert_failed="1"
red "$(date +"%Y-%m-%d %H:%M:%S")-Failed to apply for the certificate, please try to apply for the certificate manually."
    fi
    install_xray
}

install_xray(){
    green "$(date +"%Y-%m-%d %H:%M:%S") ==== install xray"
...
EOF
    ...
    green "This installation detection information is as follows, if nginx and xray start normally, it means the installation is normal:"
...
check_domain(){
    ...
    if [$real_addr == $local_addr]; then
        green "The domain name resolution address matches the VPS IP address."
        install_nginx
    else
        red "The domain name resolution address does not match the VPS IP address."
...
remove_xray(){
    green "$(date +"%Y-%m-%d %H:%M:%S")-delete xray."
...
function start_menu(){
    clear
    green "=============================================== ====="
    green "Description: xray + tcp + xtls one-click installation script"
    green "System: support centos7/debian9+/ubuntu16.04+"
    green "by atrandys www.atrandys.com"
    green "=============================================== ====="
    echo
    green "1. Install xray + tcp + xtls"
    green "2. Update xray"
    red "3. Delete xray"
    green "4. View configuration parameters"
    yellow "0. Exit"
    echo
